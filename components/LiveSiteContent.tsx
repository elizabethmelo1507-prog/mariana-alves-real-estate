...
interface LiveSiteContentProps {
    config: {
        brandColor: string;
...
const LiveSiteContent: React.FC<LiveSiteContentProps> = ({ config }) => {
    ...
    return (
        <div className="h-full overflow-y-auto hide-scrollbar relative" style={{ backgroundColor: bgColor }}>
            ...
            <nav className={`sticky top-0 z-10 backdrop-blur-md px-4 py-3 flex items-center justify-between border-b border-gray-100/10`} style={{ backgroundColor: `${bgColor}cc` }}>
                <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-sm" style={{ color: textColor }}>{config.title}</span>
                </div>
...
export default LiveSiteContent;
